...
tb.Id AS TaxBill_ID
, tr.Id AS TaxRoll_ID
, tr.Descr AS TaxRoll_Description
, tb.AssessmentYear

FROM TaxBill AS tb
JOIN TaxRoll AS tr ON tb.TaxRollId=tr.Id
)
...
LEFT JOIN CTE_FncDetails AS fndl ON kcv.lrsn=fndl.RevObjId
  AND fndl.[Total] > '0'
LEFT JOIN CTE_TaxBillInfo AS tbi ON fndl.TaxBillId=tbi.TaxBill_ID
...
AND kcv.pin LIKE 'M%'
    ...
    OR (kcv.pin LIKE 'UP%' AND kcv.ClassCD='010 Operating Property')

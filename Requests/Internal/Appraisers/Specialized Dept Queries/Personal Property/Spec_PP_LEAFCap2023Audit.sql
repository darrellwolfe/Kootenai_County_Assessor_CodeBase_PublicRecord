...
Declare @Year int = 2023; -- Input THIS year here
...
--    109 Total Value Total Value
Declare @ValuePPEX INT = 310;
-- 310	PP_Exemption	PP Exemption 602KK
Declare @ValueNetTax INT = 455;
...
JOIN tsbv_cadastre AS c 
  On c.CadRollId = r.Id
  And c.CadInvId = i.Id
  And c.ValueType = @ValuePPEX -- Variable
/*
Declare @ValueTypetotal INT = 109;
--    109 Total Value Total Value
Declare @ValuePPEX INT = 310;
...
JOIN tsbv_cadastre AS c 
  On c.CadRollId = r.Id
  And c.CadInvId = i.Id
  And c.ValueType = @ValueNetTax -- Variable
/*
Declare @ValueTypetotal INT = 109;
--    109 Total Value Total Value
Declare @ValuePPEX INT = 310;
...
,ppex.Cadaster_Value AS ppex

,nettax.Cadaster_Value AS nettax

,ct.AssessmentType

From CTE_Total AS ct

Left Join CTE_PPEx AS ppex
    On ppex.lrsn = ct.lrsn

Left Join CTE_NetTax AS nettax
    On nettax.lrsn = ppex.lrsn

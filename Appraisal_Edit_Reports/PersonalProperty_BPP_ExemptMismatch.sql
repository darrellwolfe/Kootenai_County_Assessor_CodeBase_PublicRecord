...
-- Step 1: Create a derived table with the custom logic for CheckColumn
CheckColumnTable AS (
    SELECT
        *,
        CASE
            WHEN NetTaxable IS NULL THEN 'Check'
            WHEN NetTaxable < 0 THEN 'Neg'
            WHEN NetTaxable > 0 THEN 'Pos'
            WHEN NetTaxable = 0 THEN 'Zero'
            ELSE 'Check'
        END AS CheckColumn
    FROM CTE_PP_Query_20_32
),

-- Step 2: Group by Owner and CheckColumn and count occurrences
GroupedData AS (
    SELECT
        Owner,
        CheckColumn,
        COUNT(*) AS Count
    FROM CheckColumnTable
    GROUP BY Owner, CheckColumn
),

-- Step 3: Identify duplicates in the grouping
DuplicateOwners AS (
    SELECT
        Owner
    FROM GroupedData
...
-- Final Step: Join the grouped data with duplicates to filter results
FinalResult AS (
    SELECT
        g.*
    FROM GroupedData g
    INNER JOIN DuplicateOwners d ON g.Owner = d.Owner
)

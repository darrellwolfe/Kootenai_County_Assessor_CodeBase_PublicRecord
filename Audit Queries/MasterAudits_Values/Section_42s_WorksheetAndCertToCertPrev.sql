...
DECLARE @PrimaryTransferDateFROM DATE = CAST(CAST(@TaxYear-1 AS VARCHAR) + '-01-01' AS DATE);
DECLARE @PrimaryTransferDateTO DATE = CAST(CAST(@TaxYear-1 AS VARCHAR) + '-12-31' AS DATE);
...
DECLARE @ImpMemosFromYear VARCHAR(8) = '%' + RIGHT(CAST(@TaxYear AS VARCHAR),2) + '%';
--'%2024%'
DECLARE @MemoLastUpdatedNoEarlierThan DATE = CAST(CAST(@TaxYear AS VARCHAR) + '-01-01' AS DATE);
...
SELECT
  lrsn,
  STRING_AGG(memo_text, ' | ') AS memo_text
FROM memos

WHERE status = 'A'
AND memo_id IN ('IMP')
AND memo_line_number IN ('2','3','4')
AND last_update >= @MemoLastUpdatedNoEarlierThan
AND memo_text LIKE @ImpMemosFromYear
/*
AND (memo_text LIKE '%24%'
*/
GROUP BY lrsn
)
...
,'Cert2Cert>' AS CertToCert
--Difference $
,(cv.CertValue_Land_Current - cvp.CertValue_Land_Previous) AS LandDiffAmount

,(cv.CertValue_Imp_Current - cvp.CertValue_Imp_Previous) AS ImpDiffAmount

,(cv.CertValue_Total_Current - cvp.CertValue_Total_Previous) AS TotalDiffAmount

--Difference %
,(cv.CertValue_Land_Current / NULLIF(cvp.CertValue_Land_Previous, 0)) AS LandDiffPerc

,(cv.CertValue_Imp_Current / NULLIF(cvp.CertValue_Imp_Previous, 0)) AS ImpDiffPerc

,(cv.CertValue_Total_Current / NULLIF(cvp.CertValue_Total_Previous, 0)) AS TotalDiffPerc

,'WkshtToCert>' AS WkshtToCert

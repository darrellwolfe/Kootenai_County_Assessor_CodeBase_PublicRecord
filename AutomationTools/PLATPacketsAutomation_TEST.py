...
logging.basicConfig(
    filename='S:/Common/Comptroller Tech/Reports/Python/Auto_Mapping_Packet/PlatPacketsAutomation.log',
    level=logging.DEBUG,
...
def on_submit():
    ...
    global AINLIST, AINFROM, PDESC, PFILE, PNUMBER, TREVIEW, MappingPacketType, Initials, MemoTXT, ForYear, PLATCOMBO, LANDTYPE, LEGENDTYPE

    # Collect inputs for AINFROM and split by commas
    AINFROM = [ain.strip() for ain in entry_ainfrom.get().strip().upper().split(",")]
    # Combine the AINFROM lists, removing duplicates
    combined_ain_list = list(set(AINFROM))
...
def setup_widgets(root):
    global entry_ainfrom, entry_pfile, entry_pnumber, entry_treview, combobox_mappingpackettype, entry_initials, for_year_combobox
    global entry_platcombo, combobox_landtype, entry_legendtype
    ...
    submit_button.grid(column=0, row=11, columnspan=2, pady=20)
...
rows = execute_query(cursor, query, (PLATCOMBO + '%',))

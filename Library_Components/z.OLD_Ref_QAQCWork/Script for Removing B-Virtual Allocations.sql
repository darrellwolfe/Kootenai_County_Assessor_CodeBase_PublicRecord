...
select * from reconciliation where status = 'W' and method not in ('O','R') and lrsn in (select lrsn from allocations where improvement_id = 'B' and status = 'A')
...
update  reconciliation  set  recon_override_land = 1 where status = 'W' and method not in ('O','R') and lrsn in (select lrsn from allocations where improvement_id = 'B' and status = 'A')

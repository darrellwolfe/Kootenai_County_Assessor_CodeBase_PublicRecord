...
where v.last_update > '12-31-2012' 									
	"and (v.valuation_comment in ('06- Occupancy','38- Subsequent Assessment') or v.change_reason in ('06','38'))"								
...
"select ci.cadinvctrlid, count(ci.id) as InvCnt from cadinv ci"									
where ci.cadinvctrlid > 474 and ci.begtaxyear=									
	(select max(ci_sub.begtaxyear) from cadinv ci_sub where ci.id=ci_sub.id)								
group by ci.cadinvctrlid									
order by ci.cadinvctrlid									

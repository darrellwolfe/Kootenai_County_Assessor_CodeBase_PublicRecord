...
Declare @EffYear0101ThisYear varchar(8) = Cast(@Year as varchar) + '0101'; -- Generates '20230101' for the previous year

Declare @EffYear0101LastYear varchar(8) = Cast(@YearPrev as varchar) + '0101'; -- Generates '20230101' for the previous year
...
Where i.status = 'H'
And (i.improvement_id IN ('D','M','C') AND i.year_built = @YearPrev-1)
...
From valuation AS v
Where v.status = 'A'
And v.eff_year LIKE @EffYear0101ThisYear
),

CTE_ValuationLastYear AS (
Select Distinct
v.lrsn
,v.imp_assess AS TotalAssessed_Imp
,v.land_assess AS TotalAssessed_LandValue
...
From valuation AS v
Where v.status = 'A'
And v.eff_year LIKE @EffYear0101LastYear
),
...
,vty.TotalAssessed_Imp AS CertValueThisYear
,CASE
  WHEN vly.TotalAssessed_Imp IS NULL THEN 0
  ELSE vly.TotalAssessed_Imp
END AS CertValueLastYear
--,vly.TotalAssessed_Imp AS CertValueLastYear

,CASE
  WHEN vly.TotalAssessed_Imp IS NULL THEN vty.TotalAssessed_Imp-0
  ELSE (vty.TotalAssessed_Imp - vly.TotalAssessed_Imp)
END AS ImpDiff_YearOverYear
--,(vty.TotalAssessed_Imp - vly.TotalAssessed_Imp) AS ImpDiff_YearOverYear


,cvd.ImpValDetail_GroupCode -- What about duplicates?? 
...
Left Join CTE_ValuationThisYear AS vty
  On vty.lrsn = pmd.lrsn
  And vty.RowNum = 1

Left Join CTE_ValuationLastYear AS vly
  On vly.lrsn = pmd.lrsn
  And vly.RowNum = 1
...
--  And vty.PostedValuationComment LIKE '06%'
  And cvd.ImpValDetail_YearBuilt <> cvd.PostedYear
  --And cvd.ImpValDetail_YearBuilt = cvd.PostedYear
--And pmd.lrsn = 2
  And vty.TotalAppraisedMarketValue > 0
  
  And nc.NC IS NOT NULL
  And nc.NC NOT LIKE '%EXCLUDE%'

...
def validate_yn(action, value_if_allowed):
    if action == '1':  # Insert or edit action
        if value_if_allowed == '':
            return True
        value_if_allowed = value_if_allowed.upper()
...
def setup_widgets(root):
    ...
    for_year_combobox.set("Select year")  # Set the initial text
    ...
    mapping_packet_types = [
        "MERGE", "SPLIT", "BLA", "LLA", "RW VACATION", "RW SPLIT", "REDESCRIBE", "RW AUDIT",
        "RW Cat19", "AIRPORT LEASE NEW PARCEL", "PLAT VACATION", "PARCEL DELETED", "ACERAGE AUDIT", "NEW PLAT"
    ]
...
def press_key_with_modifier_multiple_times(key, times):
...
def download_and_install_tesseract():
    ...
    if check_tesseract_installed():
        print("Tesseract has been successfully installed.")
    else:
        print("Tesseract installation could not be verified. Please ensure it's installed correctly.")
        print(f"Expected path: {get_tesseract_path()}")
        sys.exit(1)

Sub CopyRowsByGeoCode()
    ...
    For wsIndex = 1 To ThisWorkbook.Sheets.Count
        If ThisWorkbook.Sheets(wsIndex).Name = "1008" Then
            startWsIndex = wsIndex
            Exit For
        End If
    Next wsIndex

    ' Loop through each worksheet starting from "1001"
    For wsIndex = startWsIndex To ThisWorkbook.Sheets.Count
        ...
        For i = 2 To sourceLastRow
            ...
            If targetWorksheet.Cells(1, 2).Value = geoCode Then
                ...
                sourceWorksheet.Range("C" & i & ":AZ" & i).Copy
                targetWorksheet.Cells(targetRow, 1).PasteSpecial xlPasteValues
                sourceWorksheet.Cells(i, 53).Copy
                targetWorksheet.Cells(targetRow, 75).PasteSpecial xlPasteValues
                sourceWorksheet.Cells(i, 54).Copy
                targetWorksheet.Cells(targetRow, 76).PasteSpecial xlPasteValues

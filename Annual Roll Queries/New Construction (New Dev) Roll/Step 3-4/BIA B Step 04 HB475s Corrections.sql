...
And ((i.improvement_id IN ('D','M') 
    AND i.year_built BETWEEN @YearPrev-4 AND @YearPrev)

  OR (i.improvement_id LIKE 'A%' 
    AND i.year_built BETWEEN @YearPrev-4 AND @YearPrev)

  OR (i.improvement_id IN ('C') 
    AND i.year_built BETWEEN @YearPrev-4 AND @YearPrev)
...
,fnd.TIFId
,fnd.TIF
,fnd.OverrideAmount AS URD_BaseValue
...
Where fnd.NC IS NOT NULL
And fnd.ImpValDetail_ImpId IS NOT NULL
